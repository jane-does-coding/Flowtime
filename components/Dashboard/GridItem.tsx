...
const GridItem = ({ task, isLast, closeDeadline }: any) => {
	return (
		...
		>
			<Link
				href={`/tasks/${task.id}`}
				className={`border rounded-lg overflow-hidden shadow-md px-2 md:px-4 xl:px-4 py-2 md:py-4 flex flex-col gap-2 items-start border-neutral-400/50 ${
			...
			>
				<h2 className="text-lg xl:text-xl font-bold text-black jura">
					{task.title}
				</h2>
				...
				<div className="flex gap-2 flex-wrap mt-auto">
					...
					<span className="text-sm bg-red-300 text-neutral-900 px-2 xl:px-4 py-1 xl:py-2 rounded-full">
						{task.deadline}
					</span>
					<span
						className={`text-sm  px-2 xl:px-4 py-1 xl:py-2 rounded-full 
